...
public class DroneController : MonoBehaviour {
	...
	public int DroneHitPoints = 100;
	public int DroneHitPointsMAX = 100;
...
//						newRigid.drag = 0.5f;
	...
	private float droneDeathTimer = 0;
    ...
    [SerializeField]
    private Animator dronOutAnimator;
    ...
    private bool droneIsGoingToSpawnPoint = false;
    private bool droneIsGoingUp = true;

    private Vector3 droneSpawnPoint;

    public Vector3 DroneSpawnPoint {
        set {
            droneSpawnPoint = value;
        }
    }
	public void GetToSpawnPoint() {
        droneIsGoingToSpawnPoint = true;
        hoverEffect.Play();
    }

    public void DronIsUp() {
        droneIsGoingUp = false;
        droneIsGoingToSpawnPoint = true;
        dronOutAnimator.enabled = false;
	...
	void Update () {
        ...
        if(droneIsGoingToSpawnPoint) {
            transform.rotation = Quaternion.LookRotation(droneSpawnPoint);
            transform.Translate((droneSpawnPoint - transform.position) * Time.deltaTime * 0.3f, Space.World);

            if (Vector3.Distance(transform.position, droneSpawnPoint) < Random.Range(0.1f, 2f))
                droneIsGoingToSpawnPoint = false;
		...
		if (DroneAlive) {
            if(Vector3.Distance(transform.position, CurrentTarget.position) > 6) 
                transform.Translate((CurrentTarget.position - transform.position) * Time.deltaTime * 0.1f, Space.World);
	...
	private void KillDrone() {
		...
		DroneAlive = false;
        spanwer.GetComponent<EnemySpawner>().SpawnMore();
	}
			...
			if (CurrentDroneState == DroneStateTypes.Idle01 || CurrentDroneState == DroneStateTypes.Idle02 ||
			   ...
			   || CurrentDroneState == DroneStateTypes.LookBackward) {
				if (droneIdleTimer < droneIdleTimerFreq) {
					...
					int droneIdleRandomState = Random.Range (0, 5);
	...
	private void FireCannon(CannonSideTypes sideType) {
		if (sideType == CannonSideTypes.Right) {
			...
			if (CannonProjectilePrefab != null) {
				Quaternion fireDirection = Quaternion.LookRotation (CurrentTarget.position - Cannon_RightTransform.position);
			...
			if (CannonProjectilePrefab != null) {
				Quaternion fireDirection = Quaternion.LookRotation (CurrentTarget.position - Cannon_LeftTransform.position);
